...
export const OrderItemSchema = z.object({
  ...
  if (!item.trim_width_mm || !item.trim_height_mm) {
    const extracted = parseDimensionsFromFilename(item.pdf_source_url);
    if (extracted) {
      item.trim_width_mm = item.trim_width_mm ?? extracted.width_mm;
      item.trim_height_mm = item.trim_height_mm ?? extracted.height_mm;
    } else {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message: 'Format netto (trim_width_mm/trim_height_mm) jest wymagany, ponieważ nie udało się go automatycznie rozpoznać z nazwy pliku.',
      });
    }
  }
  return item as typeof item & { trim_width_mm: number; trim_height_mm: number };
});
